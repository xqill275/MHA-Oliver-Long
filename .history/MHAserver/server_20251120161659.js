...
const autoSec = require("./autoSecurityScript")
...
app.get("/api/vitals/:userID", (req, res) => {
    ...
    const sql = "SELECT * FROM vitals WHERE userID = ? ORDER BY timestamp DESC";
...
app.post("/api/vitals/update", (req, res) => {
    const { userID, temperature, heartRate, systolic, diastolic } = req.body;
    ...
    const sql = `
        INSERT INTO vitals (userID, temperature, heartRate, systolic, diastolic, timestamp)
        VALUES (?, ?, ?, ?, ?, NOW())
        ON DUPLICATE KEY UPDATE
    ...
    db.query(sql, [userID, temperature, heartRate, systolic, diastolic], (err, result) => {
        if (err) {
            console.error("Database upsert failed:", err);
            return res.status(500).json({ error: "Database error" });
        }
